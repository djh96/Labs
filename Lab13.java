...
public static long[] fibNums = {
    ...
    public static int counter = -1, origNum = 0;
    public static long memoizedFibonacci(long num) 
    {
		long value = 0, num1 = 0, num2 = 1;
    	if(counter == -1) 
    	{
    		origNum = new Long(num).intValue();
    	}
    	if(counter == -1) 
    	{
    	...
    	if(counter < origNum) 
    	{
    		Integer i = new Integer(intArray.get(counter+1).intValue() + intArray.get(counter).intValue());
    		counter++;
    		intArray.add(i);
    		memoizedFibonacci(num-1);	    		
    	}

    	for(int j = origNum-2; j < origNum; j++) 
    	{
    		value += intArray.get(j).intValue();	
    	}
    	_numMemoizedCalls++;
		return value;
    }
    ...
    {
		double ratio = 1.6180339887498948482;

		return (int) (Math.pow(ratio, (double) num )/(Math.sqrt(5)));
    }

    public static long lookupTableFibonacci(long num) 
    {
		return fibNums[new Long(num).intValue() - 1] + fibNums[new Long(num).intValue() - 2];
    }
	...
	/*
	  Results: 
	  			All together: 12.939 secs
				Recursive: 12.113 sec s
				Memoized: .084 secs
				Closed-Form: .085 secs
				Look-Up Table: .088 secs
	*/
    }
